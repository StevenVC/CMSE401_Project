...
int main(int argc, char **argv) {
    ...
    */ 
    ...
    vector<double> g_best_fit_hist (iter_max, 0);
    ...
    for (int i=0; i<n_particles; i++) {
        ...
        if (P_fit_val < g_fit_val) {
            g_fit_val = P_fit_val;
            g_best = point_swarm[i][0];
        }
    }
    
    for (int u=0; u<iter_max; u++) {
        /*
        cout << "An iteration: " << u << "\n";
        */
        for (int i=0; i<n_particles; i++) {
            ...
            for (int j=0; j<n_v_p; j++) {
                ...
                point_swarm[i][2][j] = c_1[u][i][j] * r_1[u][i][j] * (P_best[j] - point_swarm[i][0][j]) + \
            ...
            // cout << "\n" << "I have a fitness of " << g_fit_val << "\n";
            // cout << "\n";
        }

        g_best_fit_hist[u] = g_fit_val;

        if ((stop_cond == 1) && (g_fit_val < 1E-5)) {
            break;
        }

        /*
        cout << "The global best at iteration " << u << " is: ";
        for (int j=0; j<n_v_p; j++) {
            cout << g_best[j] << ", ";
        }
    ...
    */
    ofstream out_file;
    out_file.open("g_best_hist.txt");

    for (int i=0; i<iter_max; i++) {
        out_file << g_best_fit_hist[i] << "\n";
    }
    ...
    */
    out_file.open("algorithm_solution.txt"); 
    ...
    out_file << "\n" << "This solution had a fitness value of " << g_fit_val << "\n";
